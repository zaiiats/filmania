...
const StyledLogoWrapper = styled.div<{ $size: LogoSize }>`
  display: flex;
  gap: ${({ $size }) => ($size === "small" ? "0.8rem" : "1.5rem")};
  align-items: center;
...
export default function Logo({ size = "small" }: { size?: LogoSize }) {
  return (
...
export function MovieSvg({ size }: { size: LogoSize }) {
  return (
    <svg
